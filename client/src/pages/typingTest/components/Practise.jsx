...
export const Practise = () => {
    ...
    const onClickStart = () => {
        const sentence = inputRef.current.value.trim();
        if (!sentence) return;

        const generatedLesson = getMappings(sentence);
    ...
    return (
        <div className="w-full h-full p-[1rem] flex flex-col justify-center items-center">
            <div className="flex justify-center items-center w-full gap-[1rem]">
                <input
                    ref={inputRef}
                    className="bg-white p-[0.5rem] border border-gray-300 rounded-[0.2rem] focus:outline-none focus:ring-2 focus:ring-blue-500 flex-1"
                    placeholder="Enter the text you want to practise..."
                />
                <button
                    className="bg-blue-500 text-white p-[0.5rem] rounded-[0.2rem] hover:bg-blue-600"
